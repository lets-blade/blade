...
import static com.hellokaton.blade.validator.Validators.isEmail;
import static com.hellokaton.blade.validator.Validators.length;
...
public class ValidatorTest {
    ...
    static class TopicValidator {
        public static void valid(Topic param) {
            Validators.notNull().test(param).throwIfInvalid("topic");
            length(3, 20).test(param.getTitle()).throwIfInvalid("title");
            length(5, 100).test(param.getContent()).throwIfInvalid("content");
